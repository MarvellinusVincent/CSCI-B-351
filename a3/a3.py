...
def expand_fringe(current_state, fringe):
    ...
    for move in moveSet:
        if(current_state.board.slide_blank(move) != None):
            new_State = State.State(current_state.board.slide_blank(move), current_state, current_state.depth + 1, 0)
...
def breadth_first_search(fringe, max_depth, goal_board):
    goal_State = State.State(goal_board,None,0,0)
    current_Fringe = []

    for eachState in fringe:
        if(eachState.depth <= max_depth):
            current_Fringe.append(eachState)

    for current_State in current_Fringe: 
        if(current_State.__eq__(goal_State)):

            return current_State
        else:
            expand_fringe(fringe.pop(),fringe)
