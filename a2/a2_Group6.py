...
class Board():
    ...
    def evaluateSpace(self, space):
        num_constraints = set()
        num_constraints = num_constraints.union(self.valsInRows[space[0]])
        num_constraints = num_constraints.union(self.valsInCols[space[1]])
        num_constraints = num_constraints.union(self.valsInBoxes[self.spaceToBox(space[0], space[1])])
        return len(num_constraints)
    ...
    def getMostConstrainedUnsolvedSpace(self):
        ...
        for space in self.unsolvedSpaces:
            unsolved_spaces[space] = self.evaluateSpace(space)
        if not self.unsolvedSpaces:
            return None
        return max(unsolved_spaces, key = unsolved_spaces.get)
...
class Solver:
    ...
    def solveBoard(self, board):
        if not board.unsolvedSpaces:
            return True
        constrained_space = board.getMostConstrainedUnsolvedSpace()
        for i in range(1, board.n2 + 1):
            if board.isValidMove(constrained_space, i):
                board.makeMove(constrained_space, i)
                possiblesolutions = self.solveBoard(board)
                if possiblesolutions:
                    return possiblesolutions
                else:
                    board.undoMove(constrained_space, i)
        return False
